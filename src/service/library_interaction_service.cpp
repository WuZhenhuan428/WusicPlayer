...
void LibraryInteractionService::bind()
{
    ...
    if (m_bound == true) return;

    connect(m_library_widget, &LibraryWidget::sgnImportFiles, m_playlist_ctl, &PlaylistController::importFiles);
    connect(m_library_widget, &LibraryWidget::sgnImportDir, m_playlist_ctl, &PlaylistController::importDir);
    connect(m_library_widget, &LibraryWidget::sgnSwitchPlaylist, m_playlist_ctl, &PlaylistController::switchToPlaylist);
    connect(m_library_widget, &LibraryWidget::sgnRenamePlaylist, m_playlist_ctl, &PlaylistController::renamePlaylist);
    connect(m_library_widget, &LibraryWidget::sgnRemovePlaylist, m_playlist_ctl, &PlaylistController::removePlaylist);
    connect(m_library_widget, &LibraryWidget::sgnSavePlaylist, m_playlist_ctl, &PlaylistController::savePlaylist);
    connect(m_library_widget, &LibraryWidget::sgnCopyPlaylist, m_playlist_ctl, &PlaylistController::copyPlaylist);
    connect(m_library_widget, &LibraryWidget::sgnRemoveTrackRequested,
            this, [this](const trackId& tid) {
        if (tid.isNull()) {
            return;
        }
    ...
    connect(m_library_widget, &LibraryWidget::sgnPlayTrackByModelIndex,
    ...
    connect(m_playlist_ctl, &PlaylistController::playlistChanged,
            this, &LibraryInteractionService::refreshPlaylistView);
