...
class ThemeManager : public QObject, public IConfigurable {
    Q_OBJECT
public:
    enum Source { System, Builtin, External };
    ...
    // ---- 运行状态 ----
    Source currentSource() const       { return m_currentSource; }
    ...
    QStyle* currentStyle() const       { return m_currentStyle; }

    // ---- 图标模式（独立于主题的明/暗选择） ----
    enum IconMode { IconAuto, IconLight, IconDark };
    ...
    QString configSubKey() const override { return QStringLiteral("theme"); }
