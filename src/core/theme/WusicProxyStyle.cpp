                                    ...
                                    const QWidget* w) const
{
    ...
    case PE_PanelMenu: {
        ...
        strokeRound(p, opt->rect.adjusted(1, 1, -1, -1), m_palette.frameBorder,
    ...
    case PE_FrameDefaultButton:
        strokeRound(p, opt->rect.adjusted(0, 0, -1, -1), m_palette.frameBorder,
...
void WusicProxyStyle::fillRound(QPainter* p, const QRect& r, const QColor& bg, int /*radius*/) const
{
    p->fillRect(r, bg);
}

void WusicProxyStyle::strokeRound(QPainter* p, const QRect& r, const QColor& border, int /*radius*/,
                                  int w) const
{
...
void WusicProxyStyle::drawMenuItem(const QStyleOption* opt, QPainter* p) const
{
    ...
    if (sel && ena) {
        fillRound(p, opt->rect.adjusted(2, 1, 2, -1), m_palette.highlight, m_palette.menuRadius);
    }
...
void WusicProxyStyle::drawMenuBarItem(const QStyleOption* opt, QPainter* p) const
{
    ...
    if (sel && ena) {
        fillRound(p, opt->rect.adjusted(2, 2, -2, -2), m_palette.itemHover, m_palette.buttonRadius);
    }
...
void WusicProxyStyle::drawTabBarTab(const QStyleOption* opt, QPainter* p) const
{
    ...
    fillRound(p, opt->rect.adjusted(2, 2, -2, opt->rect.height() / 2), bg, m_palette.buttonRadius);
