...
int WusicProxyStyle::pixelMetric(PixelMetric m, const QStyleOption* opt,
                                  const QWidget* w) const {
    switch (m) {
    case PM_ScrollBarExtent:       return m_palette.scrollbarWidth;
    case PM_SliderThickness:       return m_palette.sliderGrooveH + 8;
    case PM_SliderLength:          return m_palette.sliderHandleW;
    case PM_TabBarTabHSpace:       return 12;
    case PM_TabBarTabVSpace:       return 4;
    case PM_ToolBarIconSize:       return 20;
    case PM_ToolBarSeparatorExtent: return 1;
    case PM_ToolBarFrameWidth:     return 0;
    case PM_CheckBoxLabelSpacing:  return 6;
    case PM_RadioButtonLabelSpacing: return 6;
    default: return QProxyStyle::pixelMetric(m, opt, w);
    }
}
...
QRect WusicProxyStyle::subControlRect(ComplexControl cc,
...
int WusicProxyStyle::styleHint(StyleHint hint, const QStyleOption* opt,
                                const QWidget* w, QStyleHintReturn* ret) const {
    switch (hint) {
...
void WusicProxyStyle::drawControl(ControlElement element,
    ...
    case CE_ProgressBarGroove: {
        ...
        if (pb && pb->maximum > pb->minimum) {
            const qreal ratio = static_cast<qreal>(pb->progress - pb->minimum)
                                / (pb->maximum - pb->minimum);
            QRect fillRect = opt->rect;
            fillRect.setWidth(static_cast<int>(fillRect.width() * ratio));
            if (fillRect.width() > 0 && fillRect.height() > 0)
                fillRound(p, fillRect, m_palette.progressBarFill, m_palette.buttonRadius);
        }
        break;
    }
...
void WusicProxyStyle::drawPrimitive(PrimitiveElement pe,
                                     const QStyleOption* opt,
                                     QPainter* p,
                                     const QWidget* w) const {
    switch (pe) {
    ...
    case PE_PanelToolBar:
        ...
        break;
    case PE_PanelMenu: {
        fillRound(p, opt->rect.adjusted(1, 1, -1, -1), m_palette.base, m_palette.menuRadius);
        strokeRound(p, opt->rect.adjusted(1, 1, -1, -1), m_palette.frameBorder, m_palette.menuRadius);
        break;
    }
    ...
    case PE_FrameDefaultButton:
        strokeRound(p, opt->rect.adjusted(0, 0, -1, -1), m_palette.frameBorder, m_palette.buttonRadius);
        break;
    ...
    case PE_IndicatorSpinDown: {
        ...
        QPainterPath tri;
        if (up) {
            tri.moveTo(cx - 3, cy + 1); tri.lineTo(cx, cy - 2); tri.lineTo(cx + 3, cy + 1);
        } else {
            tri.moveTo(cx - 3, cy - 1); tri.lineTo(cx, cy + 2); tri.lineTo(cx + 3, cy - 1);
        }
        tri.closeSubpath();
    ...
    case PE_IndicatorToolBarSeparator:
        p->setPen(QPen(m_palette.frameBorder, 1));
        p->drawLine(opt->rect.center().x(), opt->rect.top() + 4,
                    opt->rect.center().x(), opt->rect.bottom() - 4);
        break;
...
void WusicProxyStyle::drawComplexControl(ComplexControl cc,
                                          const QStyleOptionComplex* opt,
                                          QPainter* p,
                                          const QWidget* w) const {
    switch (cc) {
    case CC_Slider:      drawSlider(opt, p);           break;
    case CC_ScrollBar:   drawScrollBar(cc, opt, p, w); break;
    case CC_ComboBox:    drawComboBox(opt, p);         break;
    case CC_SpinBox:     drawSpinBox(opt, p, w);       break;
    case CC_ToolButton:  drawToolButton(opt, p);       break;
    default: QProxyStyle::drawComplexControl(cc, opt, p, w); break;
    }
}
...
void WusicProxyStyle::polish(QWidget* w) {
    ...
    if (auto* le = qobject_cast<QLineEdit*>(w))   le->setFrame(false);
    if (auto* cb = qobject_cast<QComboBox*>(w))   cb->setFrame(false);
    if (auto* sb = qobject_cast<QSpinBox*>(w))    sb->setFrame(false);
    if (auto* ds = qobject_cast<QDoubleSpinBox*>(w)) ds->setFrame(false);
    if (auto* sc = qobject_cast<QScrollBar*>(w))  sc->setAttribute(Qt::WA_Hover, true);
    if (auto* tb = qobject_cast<QToolButton*>(w)) tb->setAutoRaise(true);
...
void WusicProxyStyle::fillRound(QPainter* p, const QRect& r,
                                 const QColor& bg, int /*radius*/) const {
    p->fillRect(r, bg);
}

void WusicProxyStyle::strokeRound(QPainter* p, const QRect& r,
                                   const QColor& border, int /*radius*/, int w) const {
...
void WusicProxyStyle::drawButton(const QStyleOption* opt, QPainter* p) const {
    ...
    fillRound(p, r, bg, m_palette.buttonRadius);
    ...
    if (hasIcon && hasText) {
        ...
        // 仅图标，居中
        const int iconSize = qMin(r.height() - 6, btn->iconSize.width());
        QRect iconRect(r.center().x() - iconSize / 2, r.center().y() - iconSize / 2,
                       iconSize, iconSize);
        btn->icon.paint(p, iconRect);
    }
    ...
    if (hasText) {
        ...
        drawItemText(p, contentRect, Qt::AlignCenter | Qt::TextShowMnemonic,
                     opt->palette, enabled, btn->text, textRole);
    }
...
void WusicProxyStyle::drawToolButton(const QStyleOptionComplex* opt,
                                      QPainter* p) const {
    const bool hovered = opt->state & QStyle::State_MouseOver;
    const bool sunken  = opt->state & QStyle::State_Sunken;
    ...
    if (hovered || sunken) {
        ...
        fillRound(p, opt->rect.adjusted(2, 2, -2, -2), bg, m_palette.buttonRadius);
    }
}
...
void WusicProxyStyle::drawSlider(const QStyleOption* opt, QPainter* p) const {
    ...
    QRectF groove;
    if (horiz) {
        groove = QRectF(gr.x(), gr.center().y() - m_palette.sliderGrooveH / 2.0,
                        gr.width(), m_palette.sliderGrooveH);
    } else {
    ...
    // 未过部分
    if (horiz) {
        QRectF unfilled(filled.right(), groove.y(),
                        groove.right() - filled.right(), groove.height());
        p->fillRect(unfilled, m_palette.progressBarBg);
    } else {
        QRectF unfilled(groove.x(), groove.y(),
                        groove.width(), filled.y() - groove.y());
        p->fillRect(unfilled, m_palette.progressBarBg);
    }
...
void WusicProxyStyle::drawScrollBar(ComplexControl /*cc*/,
                                     const QStyleOptionComplex* opt,
                                     QPainter* p,
                                     const QWidget* w) const {
    ...
    auto drawArrowBtn = [&](SubControl sc, bool isUpOrLeft) {
        ...
        QPainterPath tri;
        if (horiz) {
            if (isUpOrLeft) { tri.moveTo(cx - 3, cy); tri.lineTo(cx + 3, cy - 3); tri.lineTo(cx + 3, cy + 3); }
            else            { tri.moveTo(cx + 3, cy); tri.lineTo(cx - 3, cy - 3); tri.lineTo(cx - 3, cy + 3); }
        } else {
            if (isUpOrLeft) { tri.moveTo(cx, cy - 3); tri.lineTo(cx - 3, cy + 3); tri.lineTo(cx + 3, cy + 3); }
            else            { tri.moveTo(cx, cy + 3); tri.lineTo(cx - 3, cy - 3); tri.lineTo(cx + 3, cy - 3); }
        }
        tri.closeSubpath();
        p->drawPath(tri);
    };
    ...
    if (!sr.isNull()) {
        const bool hovered = opt->state & QStyle::State_MouseOver;
        QColor handleColor = hovered
            ? m_palette.scrollbarHandle.lighter(130)
            : m_palette.scrollbarHandle;
        if (horiz)
            sr.adjust(0, 2, 0, -2);
        else
            sr.adjust(2, 0, -2, 0);
...
void WusicProxyStyle::drawMenuItem(const QStyleOption* opt, QPainter* p) const {
    ...
    if (sel && ena) {
        fillRound(p, opt->rect.adjusted(2, 1, 2, -1),
                  m_palette.highlight, m_palette.menuRadius);
    }
    ...
    drawItemText(p, tr, Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic,
                 copy.palette, ena, mi->text,
...
void WusicProxyStyle::drawMenuBarItem(const QStyleOption* opt, QPainter* p) const {
    ...
    if (sel && ena) {
        fillRound(p, opt->rect.adjusted(2, 2, -2, -2),
                  m_palette.itemHover, m_palette.buttonRadius);
    }

    const auto* mi = qstyleoption_cast<const QStyleOptionMenuItem*>(opt);
    if (mi) {
        QPalette::ColorRole role = (sel && ena) ? QPalette::HighlightedText : QPalette::Text;
        drawItemText(p, opt->rect, Qt::AlignCenter | Qt::TextShowMnemonic,
                     opt->palette, ena, mi->text, role);
    }
}
...
void WusicProxyStyle::drawHeader(const QStyleOption* opt, QPainter* p) const {
    ...
    // 背景
    QColor bg = m_palette.button;
    if (sunken)  bg = m_palette.highlight;
    else if (hovered) bg = m_palette.itemHover;
    fillRound(p, opt->rect, bg, m_palette.buttonRadius);
    ...
    p->drawLine(opt->rect.topRight() + QPoint(0, 4),
                opt->rect.bottomRight() + QPoint(0, -4));
    ...
    if (hdr) {
        ...
        if (!hdr->text.isEmpty()) {
            ...
            drawItemText(p, textRect,
                         hdr->textAlignment | Qt::AlignVCenter | Qt::TextShowMnemonic,
...
void WusicProxyStyle::drawComboBox(const QStyleOptionComplex* opt, QPainter* p) const {
    ...
    fillRound(p, opt->rect.adjusted(1, 1, -1, -1), bg, m_palette.buttonRadius);
    strokeRound(p, opt->rect.adjusted(1, 1, -1, -1), bd, m_palette.buttonRadius);
    ...
    if (!cb->currentText.isEmpty()) {
        ...
        drawItemText(p, tr, Qt::AlignLeft | Qt::AlignVCenter,
                     opt->palette, ena, cb->currentText, QPalette::Text);
    }
...
void WusicProxyStyle::drawSpinBox(const QStyleOptionComplex* opt,
                                   QPainter* p,
                                   const QWidget* w) const {
    ...
    // 1) 背景 + 边框
    fillRound(p, opt->rect.adjusted(1, 1, -1, -1), m_palette.base, m_palette.buttonRadius);
    strokeRound(p, opt->rect.adjusted(1, 1, -1, -1), m_palette.frameBorder, m_palette.buttonRadius);
...
void WusicProxyStyle::drawTabBarTab(const QStyleOption* opt, QPainter* p) const {
    ...
    fillRound(p, opt->rect.adjusted(2, 2, -2, opt->rect.height() / 2),
              bg, m_palette.buttonRadius);

    if (sel) {
        p->fillRect(QRect(opt->rect.left() + 4, opt->rect.bottom() - 2,
                          opt->rect.width() - 8, 2),
                    m_palette.highlight);
    }
...
void WusicProxyStyle::drawCheckIndicator(const QStyleOption* opt, QPainter* p) const {
    ...
    fillRound(p, r, bg, m_palette.buttonRadius);
    strokeRound(p, r, bd, m_palette.buttonRadius);
