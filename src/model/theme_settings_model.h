...
class ThemeSettingsModel : public QAbstractTableModel {
    Q_OBJECT
public:
    enum Column { ColName = 0, ColSource, ColAuthor, ColDark, ColCount };
    enum UserRole { RoleIsDark = Qt::UserRole + 1 };

    explicit ThemeSettingsModel(QObject* parent = nullptr);

    void setEntries(const QVector<ThemeEntry>& entries);
    const QVector<ThemeEntry>& entries() const { return m_entries; }
