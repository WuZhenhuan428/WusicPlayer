...
HSVPalette::HSVPalette(uint inner_radius, uint outer_radius, QWidget* parent) :
    QWidget(parent), inner_radius(inner_radius), outer_radius(outer_radius), m_hue(0.0f),
    m_saturation(0.0f), m_value(0.0f), m_cursor_rgb{0xFF, 0xFF, 0xFF}, m_cursor_x(0), m_cursor_y(0),
    m_has_cursor(false), drag_ring_mode(false), drag_rect_mode(false),
    m_cache_hue_ring_valid(false), m_cache_rect_valid(false)
{
...
void HSVPalette::update_cursor()
{
    ...
    m_cursor_x     = static_cast<uint>(
        std::lround(std::clamp(x, static_cast<double>(left), static_cast<double>(right))));
    m_cursor_y = static_cast<uint>(
...
void HSVPalette::paint_hue_ring(QPainter* painter)
{
    if (!m_cache_hue_ring_valid) {
        m_cache_hue_ring =
            QImage(outer_radius * 2, outer_radius * 2, QImage::Format_ARGB32_Premultiplied);
