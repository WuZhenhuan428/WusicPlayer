...
TextEditorDialog::TextEditorDialog(const QString& string, QWidget* parent)
    : QWidget(parent)
{
    this->initUI();
...
void TextEditorDialog::initConnections() {
    ...
    connect(m_sb_font_size, &QSpinBox::valueChanged, this, [applyEditorFont](int) {
        applyEditorFont();
    });

    connect(m_cb_font, &QFontComboBox::currentFontChanged, this, [applyEditorFont](const QFont&) {
        applyEditorFont();
    });
}
